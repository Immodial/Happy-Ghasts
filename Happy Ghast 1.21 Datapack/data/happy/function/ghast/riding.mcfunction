...
execute as @p[tag=HGRider] if predicate happy:up store result entity @n[tag=HGRidden] Motion[1] double 0.01 run scoreboard players get _CurrentRise HGGlobals
execute as @p[tag=HGRider] if predicate happy:down store result score _CurrentRise HGGlobals run data get entity @n[tag=HGRidden] Motion[1] 100
execute as @p[tag=HGRider] if predicate happy:down run scoreboard players remove _CurrentRise HGGlobals 3
execute as @p[tag=HGRider] if predicate happy:down store result entity @n[tag=HGRidden] Motion[1] double 0.01 run scoreboard players get _CurrentRise HGGlobals
scoreboard players reset _CurrentRise HGGlobals
# Place steering entity and adjust if moving forward or back
execute rotated as @s as @p[tag=HGRider] run summon marker ^ ^ ^0.02 {Tags:["HGRidingMeasure"]}
execute rotated as @s as @p[tag=HGRider] if predicate happy:forward run tp @n[type=marker,tag=HGRidingMeasure] ^ ^ ^0.05
execute rotated as @s as @p[tag=HGRider] if predicate happy:backward run tp @n[type=marker,tag=HGRidingMeasure] ^ ^ ^-0.02
